...
<main>
  ...
  <p class="lead">解析模块、23-DoF 物理机器人与正式 MCC E05 trace 放在同一页。视频中的 FR3 真正在移动，物体固定在 world；没有正式 DP 指标。</p>
  <div class="flow">
  ...
  <section>
    ...
    <p>下面两个独立视角专门检查安装口。橙色只用于突出显示显式 adapter；数值栏给出 flange/palm mesh 最近距离和固定父子关系。</p>
    <img src="fr3_leap_mount_closeup.png" alt="FR3 flange to Leap palm mount close-up audit">
  </section>
  ...
  <p class="boundary"><strong>当前边界：</strong>M0–M4/E05 视频是 MuJoCo 动力学证据，不是硬件结果；正式协议关闭 gravity 以隔离 contact control。本页只展示正式 MCC；历史 DP release 的 3 s raw compatibility trial 是 EVIDENCE_ONLY，不是正式 DP 指标。实验完整执行记为 EVALUATED，性能由 MET/NOT_MET 单独表达。</p>
</main>
</body>
</html>
"""
