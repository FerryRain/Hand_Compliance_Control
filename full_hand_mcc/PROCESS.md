...
1. 用 v4c 启动 adaptive MPC 完整 GPU 仿真；严格保留 0.05 mm 动态自碰撞阈值。
2. 数值检查四指接触率、实际表面位移、指腹角、顶部终点、全机/物体碰撞。
3. 对输出视频抽取接触、下端、中段、上端和最终帧，审核拇指可见、指腹接触、
   无手背/指甲接触、无机械臂穿模。
...
3. 使用 `fr3_capsule_100x170_bottom_to_top_route_v4c.npz` 重跑完整 GPU；
   动态自碰撞仍按 0.05 mm 严格验收。
4. 若 GPU 失败，先根据数值日志定位接触、MPC 或工作空间原因，不得交付失败视频。
