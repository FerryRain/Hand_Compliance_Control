...
1. 保持 v17 的 70% 姿态运输、8 mm 主外移、8 mm 食指局部步态和 3 mm 掌根球；
   把 25–35 mm 二段外移从 5.0 mm 微调到约 5.3 mm，跨入圆柱段。
...
3. 使用 v17 参数，把 `--palm-clearance-secondary-lift-m` 从 `0.005` 微调到
   约 `0.0053`，重跑 GPU adaptive MPC；其余硬阈值保持不变。
