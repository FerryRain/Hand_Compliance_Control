...
@dataclass(kw_only=True)
class JointRelativePositionActionCfg(BaseActionCfg):
  """Configuration for joint relative position control."""
  ...
  def build(self, env: ManagerBasedRlEnv) -> JointRelativePositionAction:
    return JointRelativePositionAction(self, env)
...
class JointRelativePositionAction(BaseAction):
  """Control joints via relative position targets."""

  def __init__(self, cfg: JointRelativePositionActionCfg, env: ManagerBasedRlEnv):
    super().__init__(cfg=cfg, env=env)
  ...
  def apply_actions(self) -> None:
    encoder_bias = self._entity.data.encoder_bias[:, self._target_ids]
    current_pos = self._entity.data.joint_pos[:, self._target_ids] + encoder_bias
