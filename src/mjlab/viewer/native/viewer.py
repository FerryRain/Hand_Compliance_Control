...
class NativeMujocoViewer(BaseViewer):
  ...
  def sync_viewer_to_env(self) -> None:
    ...
    if pert.active != 0 and pert.select > 0:
      # Apply pose perturbation for fixed/mocap-selected bodies.
      mujoco.mjv_applyPerturbPose(self.mjm, self.mjd, pert, int(self._is_paused))
    ...
    # Persist mocap edits performed by pose perturbation.
    if self.mjm.nmocap > 0:
      sim_data.mocap_pos[self.env_idx] = torch.from_numpy(self.mjd.mocap_pos).to(
        device=sim_data.mocap_pos.device
      )
      sim_data.mocap_quat[self.env_idx] = torch.from_numpy(self.mjd.mocap_quat).to(
        device=sim_data.mocap_quat.device
      )
