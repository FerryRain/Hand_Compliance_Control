...
1. 把 demo、抓取优化器和长路径搜索器中的 22/6 硬编码参数化为 23/7。
2. 先生成 FR3 的无碰撞四指初始抓取，再规划跨上端帽面的长路线。
3. 运行单元测试、CPU 规划检查、GPU MJLab 实时仿真。
4. 输出视频、抽帧审核指腹接触/顶部到达/全机无碰撞；失败则继续改。
5. 更新 #7 与 #4，更新两级 README/PROCESS，提交并直接推送 `main`。
