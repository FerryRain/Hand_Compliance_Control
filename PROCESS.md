...
v4c、v5、v6、v7 的完整 GPU 规划均被严格验收拒绝，当前在 v7 曲面坐标系运输方法上
增加曲率相关的掌根外移，形成 v8 路线后继续 GPU 仿真。任何未完成全程数值与视频审核的
输出不得交付。
...
1. 在 v7 的曲面坐标系运输基础上生成 v8：把当前法向掌根外移提高到约 15 mm，
   并以 25–30 mm 行程平滑建立偏置；静态审核后运行 GPU MJLab。
2. 若固定 15 mm 外移影响后段手指可达性，则改为随胶囊局部曲率和掌部净距变化的
   分段/优化外移，而不是放宽碰撞阈值。
