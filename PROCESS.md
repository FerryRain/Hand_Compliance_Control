...
v4c–v17 的候选均被严格验收拒绝。当前最优候选采用 70% 曲面坐标系姿态运输、分段掌根
外移、3 mm 非接触掌根可行球、多起点 MPC，以及下端曲率过渡区的局部异步食指轨迹。
v17 已推进到 37.3 mm，失败只因掌部净距 1.845 mm 小于 2.000 mm；任何未完成全程
数值与视频审核的输出不得交付。
...
1. 在 v17 参数上把二段外移从 5.0 mm 微调到约 5.3 mm，跨过下端帽面/圆柱边界；
   继续保持 2 mm 非指尖净距、3 mm 法向误差和 50° 指腹角硬阈值。
