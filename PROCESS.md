...
正在执行 #7：把现有 22 DoF（xArm6 6 + LEAP 16）实现迁移为 23 DoF
（FR3 7 + LEAP 16），并把路线终点延长到物体上端曲面。
...
1. 完成 FR3+LEAP `MjSpec` 编译，确认 23 个关节顺序、指尖 site 和碰撞 geom。
2. 把 demo、抓取优化器和长路径搜索器中的 22/6 硬编码参数化为 23/7。
3. 先生成 FR3 的无碰撞四指初始抓取，再规划跨上端帽面的长路线。
4. 运行单元测试、CPU 规划检查、GPU MJLab 实时仿真。
5. 输出视频、抽帧审核指腹接触/顶部到达/全机无碰撞；失败则继续改。
6. 更新 #7 与 #4，更新两级 README/PROCESS，提交并直接推送 `main`。
