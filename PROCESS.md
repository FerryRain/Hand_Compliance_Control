...
v4c–v23 的候选均被严格验收拒绝。当前最优候选采用 70% 曲面坐标系姿态运输、分段掌根
外移、3 mm 非接触掌根可行球、多起点 MPC，以及下端曲率过渡区的局部异步食指轨迹。
v21 首次让 37.3 mm 帧同时满足 2 mm 全机净距、3 mm 指尖法向误差和 3 mm 掌根球，
但 39.8 mm 帧食指进度超前 4.29 mm。v22/v23 的更大或多指步态会重新损害法向/净距，
已拒绝。当前保留 v21 的 10 mm 单食指步态，并增加进度优先的高权重约束修复层；
...
1. 保留 v21 的 10 mm 单食指步态和 5.3 mm 二段外移，使用新增的 300/500
   高权重进度修复层重跑 v24，目标是把 39.8 mm 帧的 4.29 mm 超前压回 4 mm
   以内；继续保持 2 mm 非指尖净距、3 mm 法向误差和 50° 指腹角硬阈值。
2. 若圆柱段或上端帽面再次失败，继续用掌根可行球和局部异步轨迹联合求解，
   不放宽碰撞、法向或自碰撞阈值。
