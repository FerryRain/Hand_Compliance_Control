...
正在执行 #7：FR3 23 DoF 迁移与 GPU smoke 已完成；已生成满足静态自碰撞余量的
底端 v4 抓取和 0.48 m 到上端帽面的 v4c 路线，当前准备重新运行完整 GPU 仿真
和视频审核。
...
1. 使用 v4c 运行 GPU MJLab 全程仿真，动态自碰撞仍按 0.05 mm 严格验收。
2. 输出视频并抽帧审核指腹接触、拇指可见、顶部到达、
   全机无物体碰撞和无手指自碰撞；失败则继续改。
3. 补 FR3 回归测试、两级 README，执行多物体/大曲率泛化测试并更新 #7/#4。
4. 分阶段更新两级 PROCESS，提交并直接推送 `main`。
