...
- 已生成 `full_hand_mcc/assets/fr3_capsule_100x170_grasp_v1.npz`：
  ...
  自碰撞 0。尚未经过 GPU 动态接触验收。

下一步：

1. 从 FR3 初始抓取搜索并审核 0.40–0.45 m 跨上端帽面的长路线。
2. 运行 CPU 规划检查、GPU MJLab 实时仿真。
3. 输出视频、抽帧审核指腹接触/顶部到达/全机无碰撞；失败则继续改。
4. 更新 #7 与 #4，更新两级 README/PROCESS，提交并直接推送 `main`。
