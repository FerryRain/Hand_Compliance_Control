...
class HeadlessCollectConfig:
    ...
    object_profiles: str = "capsule_medium,box_medium,cylinder_medium,ellipsoid_medium"
...
class TargetObjectProfileRandomizer:
    def __init__(self, env: ManagerBasedRlEnv, cfg: HeadlessCollectConfig, device: str):
        ...
        # Validate resolved geom ids against the model arrays to avoid CUDA device-side
        # assertions stemming from out-of-range indexing. Convert ids to Python ints
        # first so we can perform safe bounds checks on CPU.
        resolved_ids = [int(self.profile_to_geom_id[p]) for p in self.profiles]

        # Ensure model fields are expanded and set model reference before inspecting.
        env.sim.expand_model_fields(("geom_contype", "geom_conaffinity", "geom_rgba"))
        self.model = env.sim.model

        # Inspect geom_contype to determine number of geoms available.
        geom_contype = self.model.geom_contype
        try:
            # Use Python int conversion of shape to avoid device-side tensor ops.
            if hasattr(geom_contype, "shape"):
                num_geoms = int(geom_contype.shape[-1])
            else:
                # Fallback: attempt to read length via torch API
                num_geoms = int(torch.tensor(geom_contype).shape[-1])
        except Exception:
            # If we can't determine shape, raise a helpful error.
            raise RuntimeError(
                "Unable to determine number of geoms from model.geom_contype; "
                "inspect the model to ensure geom fields are available."
            )

        for pid, gid in zip(self.profiles, resolved_ids):
            if gid < 0 or gid >= num_geoms:
                available = ", ".join(local_geom_names)
                raise ValueError(
                    f"Resolved geom id {gid} for profile '{pid}' is out of range "
                    f"[0, {num_geoms - 1}]. Available target geoms: {available}"
                )

        # Safe to place geom ids on the requested device now that we've validated bounds.
        self.geom_ids = torch.tensor(resolved_ids, dtype=torch.long, device=device)

        geom_rgba = self.model.geom_rgba
        geom_contype = self.model.geom_contype
        geom_conaffinity = self.model.geom_conaffinity
        ...
        if geom_contype.ndim == 2:
            self.base_contype = geom_contype[0, self.geom_ids].clone()
            self.base_conaffinity = geom_conaffinity[0, self.geom_ids].clone()
        else:
            self.base_contype = geom_contype[self.geom_ids].clone()
            self.base_conaffinity = geom_conaffinity[self.geom_ids].clone()
    ...
    def randomize(self) -> None:
        ...
        if geom_contype.ndim == 2 and geom_rgba.ndim == 3:
            env_ids = torch.arange(num_envs, device=self.device, dtype=torch.long)

            geom_contype[env_ids.unsqueeze(-1), self.geom_ids.unsqueeze(0)] = 0
            geom_conaffinity[env_ids.unsqueeze(-1), self.geom_ids.unsqueeze(0)] = 0
            ...
            geom_contype[env_ids, selected_geom_ids] = self.base_contype[profile_idx]
            geom_conaffinity[env_ids, selected_geom_ids] = self.base_conaffinity[profile_idx]
        ...
        geom_contype[self.geom_ids] = 0
        geom_conaffinity[self.geom_ids] = 0
        geom_rgba[self.geom_ids] = self.base_rgba
        geom_rgba[self.geom_ids, 3] = 0.15

        selected_geom_id = int(self.geom_ids[chosen].item())
        geom_contype[selected_geom_id] = self.base_contype[chosen]
        geom_conaffinity[selected_geom_id] = self.base_conaffinity[chosen]
...
class H5DataLogger:
    ...
    def log(self, time, fsr, q, action, obj_pose, palm_pose, quality):
