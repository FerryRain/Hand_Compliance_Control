...
class CollectConfig:
    output_dir: str = "./finger_compliance_control/data"
    num_envs: int = 1
    ...
    model_num_inference_steps: int = 10
    model_env_state_value: float = 0.0
...
class H5DataLogger:
    def __init__(self, filepath: str, num_envs: int, fsr_dim: int = 16, q_dim: int = 22, action_dim: int = 16, qfrc_dim: int = 16):
        ...
        self.dsets = {
            ...
            "qfrc_actuator": self.file.create_dataset("qfrc_actuator", (0, num_envs, qfrc_dim), 
                                                      maxshape=(None, num_envs, qfrc_dim), chunks=(chunk_size, num_envs, qfrc_dim), dtype="f4"),
    ...
    def log(self, time, fsr, q, action, qfrc_actuator, obj_pose, palm_pose):
...
class LeRobotDiffusionPolicyAdapter:
    """Use a pretrained LeRobot diffusion policy in mjlab play loop."""
    ...
    def __init__(
        ...
        env_state_value: float,
    ) -> None:
        self.env = env
        self.run_device = torch.device(run_device)
        self.env_state_value = float(env_state_value)

        print(f"[INFO] Loading LeRobot diffusion model from: {model_dir}")
        self.policy = DiffusionPolicy.from_pretrained(model_dir, local_files_only=True)
        if num_inference_steps > 0:
            self.policy.diffusion.num_inference_steps = int(num_inference_steps)
        self.policy.to(self.run_device)
        self.policy.eval()
    ...
    def __call__(self, obs: env_types.VecEnvObs) -> torch.Tensor:
        ...
        # Keep state definition consistent with training: state = [q(22), fsr(16)].
        fsr = policy_obs[:, :16]
        q = self.env.scene["robot"].data.joint_pos[:, :22]
        state = torch.cat((q, fsr), dim=-1).to(self.run_device, dtype=torch.float32)

        batch: dict[str, torch.Tensor] = {"observation.state": state}
        if self.env_state_dim > 0:
            batch["observation.environment_state"] = torch.full(
                (state.shape[0], self.env_state_dim),
                self.env_state_value,
...
def run_collect(task_id: str, cfg: CollectConfig) -> None:
    ...
    if cfg.model_dir:
        raw_policy: Any = LeRobotDiffusionPolicyAdapter(
            model_dir=cfg.model_dir,
            env=env,
            run_device=device,
            num_inference_steps=cfg.model_num_inference_steps,
            env_state_value=cfg.model_env_state_value,
        )
    else:
    ...
    def step_with_logging(action: torch.Tensor):
        ...
        # 获取关节力矩 (qfrc_actuator) [num_envs, 22] -> 提取手指 16 关节
        # 假设前 6 个是机械臂基座，后 16 个是手指
        qfrc_full = env.scene["robot"].data.qfrc_actuator  # [num_envs, 22]
        qfrc_hand = qfrc_full[:, 6:22]  # 提取手指 16 个关节 [num_envs, 16]
