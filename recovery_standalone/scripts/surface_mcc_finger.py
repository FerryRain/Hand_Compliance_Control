...
class FullHandMCCFingerConfig:
    ...
    # Deployment MCC is a local execution layer, not a second posture
    # planner.  Bound every joint correction around the external nominal q so
    # a repeated force/Jacobian update cannot fold a finger onto a different
    # kinematic branch.  ``None`` preserves collection/planning behavior.
    nominal_joint_correction_limit: float | None = None
...
class FullHandMCCFingerController:
    ...
    def update(
        ...
        if cfg.nominal_joint_correction_limit is not None:
            correction_limit = float(cfg.nominal_joint_correction_limit)
            if correction_limit <= 0.0:
                raise ValueError(
                    "nominal_joint_correction_limit must be positive"
                )
            q_command = nominal_q + np.clip(
                q_command - nominal_q,
                -correction_limit,
                correction_limit,
            )
            # ``safety_command`` is used again after EMA below.  Clamp it as
            # well; otherwise the hard-overforce branch can silently bypass
            # the local-compensation contract and replace a good nominal
            # trajectory with a large retreat command.
            safety_command = nominal_q + np.clip(
                safety_command - nominal_q,
                -correction_limit,
                correction_limit,
            )
        if self.previous_command is None:
        ...
        # Keep the public controller output inside the same nominal tube even
        # when the previous EMA state or the emergency bypass was outside it.
        if cfg.nominal_joint_correction_limit is not None:
            correction_limit = float(cfg.nominal_joint_correction_limit)
            filtered_command = nominal_q + np.clip(
                filtered_command - nominal_q,
                -correction_limit,
                correction_limit,
            )
