...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "extract_recovery_state.py output (state A). Requires "
            "--motion-mode planner_inverse and --planner-file (normally the "
            "output of generate_manifold_palm_plan.py --recovery-state paired "
            "with the same state A). At the same simulator step where the "
            "collector would otherwise reanchor the object to the planner's "
            "first waypoint, the 16 hand joints are additionally teleported "
            "to state A's q_live/qvel_live via write_joint_state_to_sim "
            "(arm joints are left untouched) and the FullHandMCC contact "
            "manifold is restarted so no stale filter/integrator state from "
            "the fixed-pregrasp settle survives the teleport."
        ),
    )
    parser.add_argument(
    ...
    if args.init_h5 is not None and not planner_inverse_mode:
        raise ValueError("--init-h5 requires --motion-mode planner_inverse")
    ...
    if manifold_mode or planner_inverse_mode:
        ...
        if planner_inverse_mode:
            ...
            if args.init_h5 is not None and not args.init_h5.is_file():
                raise FileNotFoundError(f"--init-h5 file not found: {args.init_h5}")
    ...
    # Bundle-local output root, resolved from this file rather than the cwd so
    # the collector no longer has to be launched from a specific directory.
    # Override with MCC_TRAJECTORY_DIR to share one directory across trees
    # (e.g. keep writing into the main repo's data/trajectories).
    _traj_root = Path(
        os.environ.get(
            "MCC_TRAJECTORY_DIR", Path(__file__).resolve().parent.parent / "data/trajectories"
        )
    )
    output = _traj_root / f"{filename}.h5"
    logger = StreamingH5(
    ...
    init_h5_q: torch.Tensor | None = None
    init_h5_qvel: torch.Tensor | None = None
    init_h5_failure_frame = -1
    if args.init_h5 is not None:
        with h5py.File(args.init_h5, "r") as state:
            if str(state.attrs.get("format", "")) != "mcc_recovery_state_v1":
                raise ValueError(
                    f"{args.init_h5}: expected attrs['format']="
                    "'mcc_recovery_state_v1' (extract_recovery_state.py "
                    f"output), got {state.attrs.get('format')!r}"
                )
            init_h5_q_np = np.asarray(state["q_live"], dtype=np.float32)
            init_h5_qvel_np = np.asarray(state["qvel_live"], dtype=np.float32)
            init_h5_failure_frame = int(state.attrs.get("failure_frame", -1))
        init_h5_q = torch.tensor(init_h5_q_np, dtype=torch.float32, device=device)
        init_h5_q = init_h5_q.unsqueeze(0).expand(args.num_envs, -1)
        init_h5_qvel = torch.tensor(
            init_h5_qvel_np, dtype=torch.float32, device=device
        )
        init_h5_qvel = init_h5_qvel.unsqueeze(0).expand(args.num_envs, -1)
        print(
            f"[INIT-H5] loaded q_live/qvel_live from {args.init_h5} "
            f"(failure_frame={init_h5_failure_frame}); will teleport the 16 "
            "hand joints at the planner-inverse reanchor step, arm joints "
            "left untouched"
        )
            ...
            for episode_step in range(
                ...
                if planner_inverse_mode:
                    if (
                        ...
                        motion_controller.reanchor_from_current_state()
                        if init_h5_q is not None:
                            # Teleport only the 16 hand joints to state A's
                            # live posture; the arm's own 6/7 DoF are left at
                            # whatever fixed configuration the natural
                            # fixed-pregrasp settle above already reached
                            # (reanchor_from_current_state() re-derives the
                            # object's mocap pose from that *current* measured
                            # palm pose, so it is unaffected by this).
                            env.scene["robot"].write_joint_state_to_sim(
                                position=init_h5_q,
                                velocity=init_h5_qvel,
                                joint_ids=slice(hand_q_start, hand_q_start + 16),
                            )
                            env.sim.forward()
                            print(
                                f"[INIT-H5] teleported hand joints to state A "
                                f"(failure_frame={init_h5_failure_frame}) at "
                                f"simulator step {episode_step}"
                            )
                        if args.teacher_controller == "fullhand_mcc":
                            # restart_contact_manifold() already runs on every
                            # planner_inverse reanchor; with --init-h5 this is
                            # also what discards whatever contact-QP/filter
                            # state accumulated during the fixed-pregrasp
                            # settle above, which no longer matches the
                            # just-teleported hand posture.
                            policy.restart_contact_manifold()
        ...
        logger.close(
            {
                ...
                "init_h5_file": (
                    str(args.init_h5) if args.init_h5 is not None else ""
                ),
                "init_h5_failure_frame": int(init_h5_failure_frame),
