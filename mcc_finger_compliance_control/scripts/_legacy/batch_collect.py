...
def main() -> None:
    ...
    parser.add_argument(
        "--max-prep-wait-steps",
        type=int,
        default=DEFAULTS["max_prep_wait_steps"],
        help=(
            "Extra unrecorded settling time allowed after motion-start; motion "
            "and recording begin only after the FullHandMCC teacher is ready."
        ),
    )
    ...
    parser.add_argument(
        ...
        default=("rotation",),
        help="Collect one output file per object and requested motion mode.",
    )
    parser.add_argument(
        "--filter-after",
        action="store_true",
    ...
    jobs = [
        (object_id, motion_mode)
        for object_id in object_ids
        for motion_mode in args.motion_modes
    ]
    for index, (object_id, motion_mode) in enumerate(jobs):
        seed = args.seed + index * 1000 if args.seed is not None else (42 + index * 1000)
        filename = f"{object_id}_{motion_mode}_{timestamp}"
        output = DATA_DIR / f"{filename}.h5"

        if args.skip_existing and output.exists():
            print(
                f"[BATCH] [{index+1}/{len(jobs)}] SKIP "
                f"{object_id}/{motion_mode} (exists: {output})"
            )
            collected.append(output)
            continue

        print(f"\n[BATCH] [{index+1}/{len(jobs)}] {object_id}/{motion_mode}")
        ...
        rc = _run(cmd, f"collect {object_id}/{motion_mode}")
        if rc != 0:
            failed.append(f"{object_id}/{motion_mode}")
            continue
        collected.append(output)
        ...
        if args.filter_after:
            ...
            _run(cmd, f"filter {object_id}/{motion_mode}")

    print(f"\n[BATCH] {'='*70}")
    print(f"[BATCH] DONE  collected={len(collected)}/{len(jobs)}")
    if failed:
