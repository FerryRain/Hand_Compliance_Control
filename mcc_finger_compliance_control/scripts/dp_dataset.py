...
def state_fields(path: str | Path) -> tuple[tuple[str, int], ...]:
    with h5py.File(path, "r") as file:
        ...
        if schema in ("contact_geometry_planner", "contact_geometry_planner_manifold"):
...
def planner_configuration(path: str | Path) -> dict[str, int | float] | None:
    if state_schema(path) not in (
        "contact_geometry_planner", "contact_geometry_planner_manifold"
    ):
        return None
    with h5py.File(path, "r") as file:
        return {
