...
class ObjectMotionController:
    ...
    def reset(self) -> None:
        ...
        self.angular_speeds = torch.empty(
            num_envs, device=self.env.device
        ).uniform_(self.angular_speed_min, self.angular_speed_max)
        ...
        self.translation_speeds = torch.empty(
            num_envs, device=self.env.device
        ).uniform_(self.trans_speed_min, self.trans_speed_max)
        if self.trans_distance_mode == "object_extent_ratio":
            ratio_min, ratio_max = self.trans_distance_ratio_range
            ratio = torch.empty(num_envs, device=self.env.device).uniform_(
                ratio_min, ratio_max
            )
    ...
    def step(self, episode_step: int | None = None) -> bool:
        if episode_step is None:
            episode_step = self.episode_step
            self.episode_step += 1
        moving = self.motion_start <= episode_step < (
        ...
        if self.rotation_enabled:
            if self.rotation_acceleration_time_s > 0.0:
                elapsed = (episode_step - self.motion_start + 1) * self.dt
                remaining = (
                    self.motion_start + self.motion_length - episode_step
                ) * self.dt
                ramp = min(
                    1.0,
                ...
                ramp = 1.0
            self.current_angular_speeds = self.angular_speeds * ramp
...
class FullHandMCCCollectionPolicy:
    ...
    def ready_for_motion(self) -> bool:
        return bool(np.all(self.contact_calibrated))
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.002,
        help="FullHandMCC kinematic target preload along the inward surface normal.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--enable-translation",
        action="store_true",
        help="Add world-frame sinusoidal translation during the motion window.",
    )
    parser.add_argument(
        "--trans-speed-min",
        type=float,
    ...
    rot_allowed_axes = rot_cfg.get("allowed_axes", ["uniform_sphere"])
    ...
    trans_allowed_axes = trans_cfg.get("allowed_axes", ["uniform_sphere"])
    ...
    print(
        ...
        f"translation={'ON' if args.enable_translation else 'OFF'} "
        + (
    ...
    if args.viewer != "headless":
        class VisualCollectionPolicy:
            ...
            def __call__(self, obs: dict[str, torch.Tensor]) -> torch.Tensor:
                if (
                    ...
                    and not policy.ready_for_motion
                ):
                    raise RuntimeError(
                        "FullHandMCC did not settle all four fingertips before "
                        "motion-start; increase --motion-start or fix the initial grasp"
                    )
                moving = motion_controller.step(self.step_count)
                action = policy(obs)
                ...
                if self.step_count >= record_start:
        ...
        try:
            if args.viewer == "native":
                ...
                ).run(num_steps=args.trajectory_length)
            else:
                ViserPlayViewer(
                    wrapped, visual_policy, frame_rate=args.viewer_fps
                ).run(num_steps=args.trajectory_length)
        finally:
            saved_steps = max(visual_policy.step_count - record_start, 1)
            print(
                "[VISUAL-RESULT] "
        ...
        while accepted < collected_target and attempts < max_attempts:
            ...
            for episode_step in range(args.trajectory_length):
                if (
                    ...
                    raise RuntimeError(
                        "FullHandMCC did not settle all four fingertips before "
                        "motion-start; no uncalibrated trajectory was recorded"
                    )
                moving = motion_controller.step(episode_step)
